...
print('readed')
print(len(test_graph.list_graph))
# for k, v in test_graph.list_graph.items():
#     print(k, list(map(len, v.values())))

keys_ = [k for k in test_graph.list_graph.keys()][:5]

t = time.time()
for k, v in count_garlic_in_gen_for_each({k: test_graph.list_graph[k] for k in keys_}).items():
    print(k, len(v))
print(time.time() - t)

t = time.time()
hashed = set()
print(count_garlic_for_gen(
    test_graph.list_graph,
    'GCF_000006885.1_ASM688v1_genomic',
    hashed,
    garlic_pattern=GarlicPattern()
))
print(time.time() - t)
